<template>
  <FileUpload
    name="file[]"
    :url="`${baseURL}/upload_img${paramsStringToUpload}`"
    @before-send="beforeUploadFileInput"
    :onUpload="onUpload"
    class=" "
    :multiple="true"
    accept=".png,.jpg, .gif"
    ...
    >
      ...
      >
        <div class="flex gap-2">
          ...
          ></Button>
          <Button
            label="Добавить из существующих"
            @click="visibleImage = true"
          ></Button>
        </div>
          ...
          <div class="flex-column flex gap-2">
            ...
            <div class="flex flex-column gap-2">
              ...
              <InputNumber
                ...
                :min="0"
                :max="files.length"
                @input="(e) => arraymove(files, index, e.value)"
              />
            </div>
            <Button
              icon="pi pi-times"
          ...
          >
            ...
            <div class="flex flex-column gap-2">
              ...
              <InputNumber
                ...
                :min="0"
                :max="files.length"
                @input="(e) => imageMove(index, e.value)"
              />
            </div>
            <Button
              style="width: 3rem"
  ...
  >
    <div class="grid gap-2 align-content-center justify-content-center">
      <div
        class="relative tw-select-none tw-cursor-pointer"
        v-for="image in storeImage.paginationImagesApi.data"
        :key="image"
      >
        <Button
          v-if="image.select"
          rounded
          icon="pi pi-check "
          class="absolute top-3 tw-z-20"
        />
        <Image
          @click="() => (image.select = !image.select)"
          width="150"
          height="150"
          :src="image.image_url"
        />
...
const props = defineProps({
  images: Array,
  paramsStringToUpload: { default: "", type: String },
});
// const generate
const files = ref([]);
const images = ref([]);
...
const importToPage = () => {
  images.value = [
    ...images.value,
    ...storeImage.paginationImagesApi.data.filter((e) => e.select),
  ];
};
