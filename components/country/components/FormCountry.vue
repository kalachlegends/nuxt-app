<template>
  <form @onSubmit="onFinish">
    <div class="field">
      <label for="name">Название</label>
      <InputText
        id="name"
        v-model.trim="formStateComp.name"
        required="true"
        autofocus
        :class="{ 'p-invalid': getMessageForError($vComp, 'name') }"
      />
      <small class="p-invalid" v-if="getMessageForError($vComp, 'name')">{{
        getMessageForError($vComp, "name")
      }}</small>
    </div>

    <div class="flex">
      <Button
        @click="cancel"
        label="Закрыть"
        icon="pi pi-times"
        class="p-button-text"
      />
      <Button
        type="submit"
        :loading="props.mode == 'edit' ? isLoadUpate : isLoad"
        label="Сохранить"
        icon="pi pi-check"
        class="p-button-text"
        @click="onFinish"
      />
    </div>
  </form>
</template>
<script setup>
import Dropdown from "primevue/dropdown";
import { computed, ref } from "vue";

import { useQueryBase } from "@/hooks/query/useQueryBase";
import { useCountryStore } from "../stores/country";
import { getMessageForError } from "@/helper/other/getMessageForError";
import { statusArhive } from "@/helper/status/defaultsStatus";
const props = defineProps({
  formState: Object,
  cancel: Function,
  mode: {
    type: String,
    default: "create",
  },
});
const emit = defineEmits(["create"]);
const formState = ref({ name: "" });
const formStateComp = computed(() => {
  if (props.mode == "edit") {
    return props.formState;
  }
  return formState.value;
});
const openNotificationWithIcon = (type) => {};
const store = useCountryStore();
const rules = {
  name: {
    required: true,
    validateApi: "name",
  },
};
const [createCountry, data, isLoad, $v] = useQueryBase(
  "post",
  "/country",
  {},
  {
    onSuccsess: async ({ data }) => {
      // console.log(res);
      formStateComp.value = {};
      emit("create");
      store.country.push(data.data);
      Object.keys(formStateComp.value).forEach((key) => {
        formStateComp.value[key] = null;
      });
    },
    isValidate: true,
    isMessageSucsess: true,

    rulesComp: computed(() => rules),
    isMessageError: true,
  }
);

const [updateCountry, dataUpdate, isLoadUpate, $vUpdate] = useQueryBase(
  "patch",
  "/country/:id",
  {},
  {
    isLoadGetComp: isLoad,
    rulesComp: computed(() => rules),
    isValidate: true,
    isMessageSucsess: true,
    onSuccsess: async ({ data }) => {
      // console.log(res);

      emit("create");
      let country = store.country.find((e) => e.id == data.data.id);
      Object.keys(country).forEach((key) => {
        country[key] = data.data[key];
      });
    },
    isMessageError: true,
  }
);

const $vComp = computed(() => {
  if (props.mode == "edit") {
    return $vUpdate.value;
  }
  return $v.value;
});
const onFinish = async (e) => {
  console.log(formStateComp.value);
  if (props.mode == "edit") {
    await updateCountry(
      {
        id: formStateComp.value.id,
      },
      formStateComp.value
    );
    return;
  }
  await createCountry({}, formStateComp.value);
};
</script>
